...
helpers.msg = {
	error: (error, stderr) => {
		return `@veams/cli :: ERROR: Something goes wrong! Please open an issue on github with the following error code: ${chalk.magenta(error) || chalk.magenta(stderr)}`
	},
	warning: (warning) => {
		return `@veams/cli :: WARNING: ${warning}`
	},
	info: (info) => {
		return `@veams/cli :: INFORMATION: ${info}`
	},
	help: (help) => {
		return `@veams/cli :: HELP: ${help}`
	},
	success: (item) => {
		return `@veams/cli :: DONE: ${item} successfully added!`
	}
};
...
helpers.resolveImportPath = function (firstPath, secondPath) {
	let fPath = path.dirname(firstPath);
	let sPath = path.dirname(secondPath);
